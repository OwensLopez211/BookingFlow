...
export interface AlertThresholds {
  maxFailureRate: number; // Porcentaje máximo de fallos permitido
  maxConsecutiveFailures: number; // Máximo de fallos consecutivos
  criticalErrorPatterns: string[]; // Patrones de error que requieren alerta inmediata
}
...
export class AlertService {
  ...
  async analyzeAndGenerateAlerts(
    ...
    try {
      ...
      const criticalErrors = errors.filter(error => 
        this.thresholds.criticalErrorPatterns.some(pattern => 
   ...
   */
  async sendAlerts(alerts: CriticalAlert[]): Promise<{ sent: number; failed: number; errors: string[] }> {
   ...
   */
  async checkConsecutiveFailures(organizationId: string): Promise<CriticalAlert | null> {
   ...
   */
  async detectFraudPatterns(notifications: NotificationEvent[]): Promise<CriticalAlert[]> {
