...
const transbank_1 = require("./functions/transbank");
...
const sseConnections = new Map(); // orgId -> Set of response objects
...
app.post('/v1/auth/login', async (req, res) => {
    try {
        ...
        const token = jsonwebtoken_1.default.sign({ userId: user.id, email: user.email, role: user.role, orgId: user.orgId }, JWT_SECRET, { expiresIn: '24h' });
...
app.get('/v1/notifications/stream/:orgId', (req, res) => {
    try {
...
app.post('/v1/notifications/send', authenticateToken, (req, res) => {
    try {
...
app.get('/v1/notifications', authenticateToken, (req, res) => {
    try {
        const user = req.user;
...
app.put('/v1/notifications/:id/read', authenticateToken, (req, res) => {
    try {
        const { id } = req.params;
        const user = req.user;
        const notificationIndex = notificationsHistory.findIndex(n => n.id === id && n.orgId === user.orgId);
...
app.put('/v1/notifications/mark-all-read', authenticateToken, (req, res) => {
    try {
        const user = req.user;
        let markedCount = 0;
...
app.get('/v1/notifications/unread-count', authenticateToken, (req, res) => {
    try {
        const user = req.user;
        const unreadCount = notificationsHistory.filter(n => n.orgId === user.orgId && !n.isRead).length;
        res.json({
            success: true,
            unreadCount
        });
...
const saveNotificationToHistory = (orgId, notification) => {
...
const broadcastNotificationToOrg = (orgId, notification) => {
    const connections = sseConnections.get(orgId);
    if (!connections || connections.size === 0) {
        console.log(`📡 No active connections for org: ${orgId}`);
        return 0;
...
const processNotification = (orgId, notification) => {
...
app.get('/v1/organizations/me', authenticateToken, (req, res) => {
    try {
...
app.put('/v1/organizations/:orgId/settings', authenticateToken, (req, res) => {
    try {
        const { orgId } = req.params;
...
app.get('/debug/subscriptions', async (req, res) => {
    try {
        const { mockSubscriptionRepository } = await Promise.resolve().then(() => __importStar(require('./mocks/subscriptionRepositoryMock')));
...
app.get('/onboarding/status', authenticateToken, (req, res) => {
    try {
        const user = users.find(u => u.id === req.user.userId);
        if (!user) {
            return res.status(404).json({ success: false, error: 'Usuario no encontrado' });
...
app.post('/onboarding/update', authenticateToken, (req, res) => {
    try {
                ...
                switch (stepNumber) {
                    ...
                    case 2: // Organization Setup
                        if (stepData.businessName)
                            org.name = stepData.businessName;
                        if (stepData.businessAddress)
                            org.address = stepData.businessAddress;
                        if (stepData.businessPhone)
                            org.phone = stepData.businessPhone;
                        if (stepData.businessEmail)
                            org.email = stepData.businessEmail;
                        if (stepData.currency)
                            org.currency = stepData.currency;
                        if (stepData.timezone)
                            org.settings.timezone = stepData.timezone;
                    ...
                    case 3: // Business Configuration
                        ...
                        if (stepData.appointmentModel)
                            org.settings.businessConfiguration.appointmentModel = stepData.appointmentModel;
                        if (stepData.allowClientSelection !== undefined)
                            org.settings.businessConfiguration.allowClientSelection = stepData.allowClientSelection;
                        if (stepData.bufferBetweenAppointments)
                            org.settings.businessConfiguration.bufferBetweenAppointments = stepData.bufferBetweenAppointments;
                        if (stepData.maxAdvanceBookingDays)
                            org.settings.businessConfiguration.maxAdvanceBookingDays = stepData.maxAdvanceBookingDays;
                        break;
                    case 4: // Services Setup
                        if (stepData.services && Array.isArray(stepData.services)) {
                            org.settings.services = stepData.services.map((service) => ({
                    ...
                    case 5: // Plan Selection
                        if (stepData.planId) {
                            ...
                            const getPlanLimits = (planId) => {
                                switch (planId) {
                                    case 'basic':
                                        return { plan: 'basic', maxResources: 5, maxAppointmentsPerMonth: 1000, maxUsers: 2 };
                                    case 'professional':
                                        return { plan: 'professional', maxResources: 10, maxAppointmentsPerMonth: 2500, maxUsers: 5 };
                                    case 'enterprise':
                                        return { plan: 'enterprise', maxResources: -1, maxAppointmentsPerMonth: -1, maxUsers: -1 };
                                    default:
                                        return { plan: 'basic', maxResources: 5, maxAppointmentsPerMonth: 1000, maxUsers: 2 };
                                }
                            };
...
app.post('/onboarding/reset', authenticateToken, (req, res) => {
    try {
        const userIndex = users.findIndex(u => u.id === req.user.userId);
        if (userIndex === -1) {
            return res.status(404).json({ success: false, error: 'Usuario no encontrado' });
...
app.get('/public/organization/:orgId/availability/daily-counts', (req, res) => {
    try {
        ...
        for (const date of datesArray) {
            ...
            const existingAppointments = appointments.filter(apt => apt.organizationId === orgId && apt.date === date && apt.status !== 'cancelled' && apt.status !== 'no_show');
            ...
            for (let minutes = startMinutes; minutes + duration <= endMinutes; minutes += duration + buffer) {
                ...
                if (appointmentModel === 'resource_based') {
                    ...
                    if (professionalId) {
                        ...
                        const professionalBooked = appointmentsAtTime.some(apt => apt.professionalId === professionalId);
...
app.post('/public/organization/:orgId/appointments', (req, res) => {
    try {
        ...
        const existingAppointments = appointments.filter(apt => apt.organizationId === orgId && apt.date === appointmentData.date && apt.status !== 'cancelled' && apt.status !== 'no_show');
        ...
        if (appointmentModel === 'resource_based') {
            ...
            if (appointmentData.professionalId) {
                ...
                const professionalBooked = appointmentsAtTime.some(apt => apt.professionalId === appointmentData.professionalId);
...
app.get('/appointments', authenticateToken, (req, res) => {
    try {
        ...
        if (staffId) {
            filteredAppointments = filteredAppointments.filter(apt => apt.staffId === staffId || apt.professionalId === staffId);
        }
        // Filter by resource
        if (resourceId) {
...
app.get('/v1/transbank/oneclick/status/:organizationId', authenticateToken, async (req, res) => {
    try {
        ...
        const oneclickSubscription = devSubscriptions.find((sub) => sub.organizationId === organizationId &&
            sub.oneclick_active &&
...
app.post('/v1/transbank/start-free-trial', authenticateToken, async (req, res) => {
    try {
        ...
        const { transbankService } = require('./services/transbankService');
        const subscription = await transbankService.startFreeTrial(planId, organizationId, trialDays, user.email);
        res.json({
            success: true,
...
app.use('/v1/transbank', async (req, res) => {
    try {
        ...
        const lambdaResponse = await (0, transbank_1.handler)(lambdaEvent);
