                    ...
                    case 5: // Plan Selection
                        if (stepData.planId) {
                            ...
                            const getPlanLimits = (planId) => {
                                switch (planId) {
                                    case 'free':
                                        return { plan: 'free', maxResources: 1, maxAppointmentsPerMonth: 100, maxUsers: 1 };
                                    case 'basic':
                                        return { plan: 'basic', maxResources: 5, maxAppointmentsPerMonth: 1000, maxUsers: 2 };
                                    case 'premium':
                                        return { plan: 'premium', maxResources: 10, maxAppointmentsPerMonth: 2500, maxUsers: 10 };
                                    default:
                                        return { plan: 'free', maxResources: 1, maxAppointmentsPerMonth: 100, maxUsers: 1 };
                                }
                            };
...
app.post('/public/transbank/oneclick/complete-inscription', async (req, res) => {
    try {
        ...
        if (!inscriptionResult.success) {
            return res.status(400).json({
                success: false,
                error: 'La inscripción OneClick no pudo ser completada'
            });
        }
